...
struct sender
{
  ...
  int seq;
  struct pkt lastSentPacket; // keep the last sent pkt for retransmission
  float estimate_rtt;
} A;

struct receiver
{
  int seq; // expected received seq
} B;
...
void A_output(struct msg message) {
  if (A.sstate != WAIT_LAYER5) {
    printf("A_output: waiting for ACK, drop the message %s\n", message.data);
    return;
  }

  // If we are at WAIT_LAYER5, we form the pkt to layer3
  printf("A_output: sending packet %s\n", message.data);
  struct pkt spacket;
  spacket.seqnum = A.seq;
  // we don't define ack number at sender packet A for unidirectional
  memmove(spacket.payload, message.data, 20);
  spacket.checksum = getCheckSum(spacket);
  A.lastSentPacket = spacket;
  // after sent packet to layer 3 and start timer, A starts to wait for ACK
  tolayer3(AEntity, spacket);
...
void A_input(struct pkt packet) {
  if (A.sstate != WAIT_ACK) {
    printf("A_input: not waiting for ACK, drop packet.\n");
    return;
  }
  if (packet.checksum != getCheckSum(packet)) {
    printf("A_input: ACK corrupted, drop packet.\n");
    return;
  }
  // A.seq last sent seq should equal ack from B
  if (packet.acknum != A.seq) {
    printf("A_input: ACK duplicated, drop packet.\n");
    return;
  }
  // if ACK is correct, we stop the timer and update A struct
  printf("A_input: ACKed from B.\n");
  stoptimer(AEntity);
  A.seq = 1 - A.seq;
  A.sstate = WAIT_LAYER5;
}
...
void A_timerinterrupt() {
  if (A.sstate != WAIT_ACK) {
    printf("A_timerinterrupt: waiting for ACK, ignore.\n");
    return;
  }
  printf("A_timerinterrupt: timout, retransmit last packet: %s\n.", A.lastSentPacket.payload);
  tolayer3(AEntity, A.lastSentPacket);
  starttimer(AEntity, A.estimate_rtt);
}
...
void B_input(struct pkt packet) {
  ...
  if (packet.checksum != getCheckSum(packet)) {
    printf("B_input: packet from A is corrupted, send NAK\n");
    send_ack(BEntity, 1 - B.seq);
    return;
  }
  if (packet.seqnum != B.seq) {
    printf("B_input: duplicated packet, send ACK\n");
    send_ack(BEntity, 1 - B.seq);
    return;
  }
  printf("B_input: valid packet received: %s\n", packet.payload);
  printf("B_input: send ACK\n");
  send_ack(BEntity, B.seq);
  // form a message to send to layer 5
  struct msg rmessage;
  memmove(rmessage.data, packet.payload, 20);
